...
time_left() {
	...
	local dob_epoch=$($date_bin -d "$dob" +%s 2>/dev/null) || {
		echo "Usage: time_left YYYY-MM-DD [years]"
		return 1
	}
	...
	if ((r_days <= 0)); then
		echo "0 years 0 months"
		return
	fi

	# Calculate years and remaining months
	local x_years=$(((r_days * 100) / 36525))
	local rem_days=$((r_days - (x_years * 36525 / 100)))
	local y_months=$(((rem_days * 100) / 3044))

	echo "${x_years} years ${y_months} months"
}
