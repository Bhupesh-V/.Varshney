...
dot() {
	...
	edit)
		shift
		"${g[@]}" ls-files | fzf -m --preview 'bat --style=numbers --color=always --line-range :500 {}' | xargs -r ${EDITOR:-nvim}
		;;
	open)
		local t="/tmp/dotfiles-tree"
		mkdir -p "$t"
		"${g[@]}" ls-files | while read -r f; do mkdir -p "$t/$(dirname "$f")" && ln -sf "$HOME/$f" "$t/$f"; done
		${EDITOR:-code} "$t"
		;;
	get)
		shift
